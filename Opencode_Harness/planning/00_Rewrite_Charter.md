...
贯穿人物是一位从零学习 Harness 的读者。文章可以用“请解释这个项目中的 Harness 学习入口”帮助读者理解一次请求的内部变化，但它只是解释架构的案例，不是要求读者照着执行的动手任务。

这个案例依次帮助说明 OpenCode 如何：
...
示例不得依赖读者理解缓存并发、复杂测试体系或特定业务领域。主系列以原理和源码讲解为主，不在章节末尾设置 Prompt、操作步骤、观察结果或完成条件等动手任务。
...
主系列按文件逐篇修改。每轮只选择一篇文章作为当前样板，完成结构、详细度、源码和事实审核后先由用户确认；未经确认，不批量套用到其余文章。

`07_Agent_Loop.md` 是本轮格式样板。它确认后，再按相同写作原则而不是相同标题模板处理 06、08-12。
